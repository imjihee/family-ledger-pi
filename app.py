...
</style></head><body><header><div class=top><div><div class=brand>Household Ledger</div><div class=sub>우리 가족의 지출을 한눈에</div></div><div>⌂</div></div></header><main class=wrap><div class=toolbar><label>조회 월</label><input id=month type=month><select id=user><option value="">모든 사용자</option><option value="8631664727">지희</option><option value="8806961376">광래</option></select><select id=category><option value="">모든 카테고리</option><option>식비</option><option>여가</option><option>통신/구독</option><option>경조사</option><option>쇼핑</option><option>주거/생활</option><option>저축/투자</option><option>커피</option><option>기타</option></select><button onclick=load() style="border:0;border-radius:9px;padding:10px 16px;background:#4338ca;color:white">조회</button></div><section class=cards><div class=card><div class=label>선택 기간 지출</div><div id=total class=value>₩0</div></div><div class=card><div class=label>거래 건수</div><div id=count class=value>0건</div></div></section><section class=grid><div class=panel><h3>월별 지출 추이</h3><div class=chartbox><canvas id=monthly></canvas></div></div><div class=panel><h3>카테고리별 비율</h3><div class=chartbox><canvas id=cat></canvas></div></div><div class=panel><h3>사용자별 지출</h3><div class=chartbox><canvas id=users></canvas></div></div><div class=panel><h3>카테고리별 합계</h3><div id=catlist></div></div></section><section class="panel" style="margin-top:14px"><h3>최근 지출</h3><div class=tablewrap><table><thead><tr><th>날짜</th><th>사용자</th><th>내용</th><th>카테고리</th><th>금액</th><th></th></tr></thead><tbody id=rows></tbody></table></div></section></main><script>
(function(){
 var month=document.getElementById('month');
 month.value=new Date().toISOString().slice(0,7);
 function won(n){return '₩'+Number(n||0).toLocaleString('ko-KR');}
 function load(){
  var m=month.value,u=document.getElementById('user').value,c=document.getElementById('category').value;
  Promise.all([fetch('/api/statistics?month='+m),fetch('/api/expenses?month='+m+'&user='+encodeURIComponent(u)+'&category='+encodeURIComponent(c))]).then(function(rs){return Promise.all([rs[0].json(),rs[1].json()]);}).then(function(data){
...
var cats={};var users={};ex.forEach(function(x){var c=x.category||'기타',u=x.name||'미지정';cats[c]=(cats[c]||0)+Number(x.amount||0);users[u]=(users[u]||0)+Number(x.amount||0);});
   ...
   if(typeof Chart!=='undefined'){var us={};ex.forEach(function(x){us[x.name||'미지정']=(us[x.name||'미지정']||0)+Number(x.amount||0);});if(window._cat)window._cat.destroy();window._cat=new Chart(document.getElementById('cat'),{type:'doughnut',data:{labels:Object.keys(cats),datasets:[{label:'카테고리별 지출',data:Object.values(cats),backgroundColor:['#4f46e5','#06b6d4','#f59e0b','#ef4444','#10b981','#8b5cf6']}]},options:{responsive:true,maintainAspectRatio:false}});if(window._users)window._users.destroy();window._users=new Chart(document.getElementById('users'),{type:'bar',data:{labels:Object.keys(us),datasets:[{label:'사용자별 지출',data:Object.values(us),backgroundColor:'#4f46e5'}]},options:{responsive:true,maintainAspectRatio:false}});if(window._monthly)window._monthly.destroy();window._monthly=new Chart(document.getElementById('monthly'),{type:'bar',data:{labels:(st.monthly||[]).map(function(x){return x.label;}),datasets:[{label:'월별 지출',data:(st.monthly||[]).map(function(x){return x.total;}),backgroundColor:'#06b6d4'}]},options:{responsive:true,maintainAspectRatio:false}});}
   document.getElementById('rows').innerHTML=ex.map(function(x){return '<tr><td><input id=\"d'+x.id+'\" type=date value=\"'+x.spent_at+'\" disabled></td><td>'+((x.name)||'-')+'</td><td><input id="c'+x.id+'" value="'+((x.merchant)||x.description)+'" style="width:130px;padding:7px" disabled></td><td><select id="g'+x.id+'" style="padding:7px" disabled><option>식비</option><option>여가</option><option>통신/구독</option><option>경조사</option><option>쇼핑</option><option>주거/생활</option><option>저축/투자</option><option>커피</option></select></td><td><input id="a'+x.id+'" type=text inputmode=numeric value="'+x.amount+'" style="width:100px;padding:7px" disabled></td><td><button onclick="window.editExpense('+x.id+')">수정</button> <button onclick="window.deleteExpense('+x.id+')">삭제</button></td></tr>';}).join('');ex.forEach(function(x){var q=document.getElementById('g'+x.id);if(q)q.value=x.category||'식비';});
  }).catch(function(e){console.error(e);});
 }
 window.deleteExpense=function(id){if(confirm('이 지출을 삭제할까요?'))fetch('/api/expenses/'+id,{method:'DELETE'}).then(load);};
 window.editExpense=function(id){var c=document.getElementById('c'+id),a=document.getElementById('a'+id),g=document.getElementById('g'+id),d=document.getElementById('d'+id),b=event&&event.target;if(c){c.disabled=false;a.disabled=false;g.disabled=false;d.disabled=false;if(b){b.textContent='저장';b.onclick=function(){window.saveExpense(id);};}}};
 window.saveExpense=function(id){var content=document.getElementById('c'+id).value,amount=Number(document.getElementById('a'+id).value),category=document.getElementById('g'+id).value;if(!content||!amount)return;fetch('/api/expenses/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:content,amount:amount,category:category,spent_at:document.getElementById('d'+id).value})}).then(load);};
 window.load=load; load();
    ...
    @dashboard_auth
    def expenses_api():
        return jsonify(query_expenses(request.args.get("month"), request.args.get("user"), request.args.get("category")))
    ...
    def update_api(expense_id):
        ...
        if not data.get("content") or data.get("category") not in {"식비","여가","통신/구독","경조사","쇼핑","주거/생활","저축/투자","커피"} or not isinstance(data.get("amount"), int): return jsonify(error="invalid input"),400
        update_expense(expense_id,data["content"],data["category"],data["amount"],data.get("spent_at")); return jsonify(result="ok")
